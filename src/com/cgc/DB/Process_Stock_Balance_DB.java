    ...
    public void generater_transaction_stock_process(String date_from, String date_to) throws Exception {
        ...
        try (Connection con = new DBConnect().openConnection_CMMS_Y()) {
            ...
            while (rs.next()) {
                ...
                String SQL_Update_Mpart = " update m_part set quantity = ?,price = ?"
                        + ",update_date = ? where part_id = '" + rs.getString("part_id") + "' ;";
                System.out.println("SQL_Update_Mpart = " + SQL_Update_Mpart);

                p2 = con.prepareStatement(SQL_Update_Mpart);
                p2.setString(1, rs.getString("qty"));
                p2.setString(2, rs.getString("price_unit"));
                p2.setTimestamp(3, ts);
                p2.executeUpdate();
